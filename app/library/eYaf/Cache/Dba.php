...
class Dba extends CacheAbstract
{
	...
	{
		return unlink($this->options['dba_file']);
	}
	...
	protected function _handler($mode = 'r')
	{
	    ...
	    if (null === $this->$handler) {
	        switch ($mode) {
    	        case 'r':
    	            $this->_rHandler = dba_popen($this->options['dba_file'], 'r', $this->options['dba_type']);
    	            break;
    	        case 'w':
    	            $this->_wHandler = dba_open($this->options['dba_file'], 'c', $this->options['dba_type']);
    	            $this->_rHandler = $this->_wHandler;
    	            break;
    	        default:
    	            throw new CacheException('Not support mode, mode must be r or w.');
    	    }
	    }
	    return $this->$handler;
	}
